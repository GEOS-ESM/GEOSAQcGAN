...
if __name__ == "__main__":
    ...
    if args.mode == "val":
        # open predictions npz file
        val_npz_file = f"{args.exp_dir}/val_ens_stats_days{args.n_passes}_level{args.vertical_level}.npz"
        val_dict = np.load(val_npz_file)

        val_mean = val_dict[f"ens_mean_val"]
        
        # loop over fcst init times

        Parallel(n_jobs=8)(delayed(write_val_file)(
                            ft, fcst_init_times, fcst_lead_times, 
                            args.exp_dir, meta_data, 
                            predictions_mean, val_mean
